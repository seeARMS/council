...
function renderStatusLine(result, colorEnabled) {
  if (result.status === 'ok') {
    return style(`[ok]   ${result.name} (${formatDuration(result.durationMs)})`, '32', colorEnabled);
  }

  if (result.status === 'missing') {
    return style(`[skip] ${result.name}: ${result.detail}`, '33', colorEnabled);
  }

  if (result.status === 'timeout') {
    return style(`[time] ${result.name}: ${result.detail}`, '31', colorEnabled);
  }

  return style(`[err]  ${result.name}: ${result.detail}`, '31', colorEnabled);
}

function renderSummaryLine(result, colorEnabled) {
  if (result.status === 'ok') {
    return style(`[ok]   synthesis via ${result.name} (${formatDuration(result.durationMs)})`, '32', colorEnabled);
  }

  return style(`[err]  synthesis via ${result.name ?? 'none'}: ${result.detail}`, '31', colorEnabled);
}
